...
export let ParentHomePage = () => {
  ...
  let [hasChildCards, setHasChildCards] = useState(false);
  ...
  useEffect(() => {
    const childCardsExist = Array.from(reachableAndImplemented).some(kmid => {
      let node = knowledgeGraph.getNodeData(kmid);
      return !node.forTeachers;
    });
    setHasChildCards(childCardsExist);
  }, [reachableAndImplemented]);
